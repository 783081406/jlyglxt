...
<div id="dlg" class="easyui-dialog" style="width:400px;height:300px;padding:10px 20px" closed="true"
    ...
    <form id="fm" method="post" enctype="multipart/form-data">
        ...
        <div class="fitem">
            <label>标题:</label>
            <input name="btitle" class="easyui-validatebox" required="required">
        </div>
        <div class="fitem">
            <label>内容:</label>
            <input style="height: 90px" name="bcontent" class="easyui-textbox" data-options="multiline:true"
                   required="required">
        </div>
    </form>
</div>
...
<script>
    ...
    function newb() {
        ...
        $('#fm').form('clear');
        //提交数据处理的URL
        url = '<%=basePath %>billboardsAction/saveInformation.action';
    }
    ...
    function removeb() {
        ...
        if (row) {
            $.messager.confirm('警告', '是否确定删除本行数据?', function (r) {
                ...
                if (r) {
                    //post提交
                    $.post('<%=basePath %>billboardsAction/removeInformation.action', {bid: row.bid}, function (result) {
                        if (result.success) {
                            $('#dg').datagrid('reload');	// 重新加载数据
                        } else {
                            $.messager.show({	// 显示错误的信息
