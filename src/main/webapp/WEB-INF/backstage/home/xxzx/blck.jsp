    ...
    function removebl() {
        ...
        if (row) {
            $.messager.confirm('警告', '是否确定删除本行数据?', function (r) {
                ...
                if (r) {
                    //post提交
                    $.post('<%=basePath %>chaction/removeInformation.action', {chId: row.chId}, function (result) {
                        if (result.success) {
                            $('#blt').datagrid('reload');	// 重新加载数据
                        } else {
    ...
    function removexd() {
        ...
        if (row) {
            $.messager.confirm('警告', '是否确定删除本行数据?', function (r) {
                ...
                if (r) {
                    //post提交
                    $.post('<%=basePath %>ecgiaction/removeInformation.action', {ecgId: row.ecgId}, function (result) {
                        if (result.success) {
                            $('#xdt').datagrid('reload');	// 重新加载数据
                        } else {
                            $.messager.show({	// 显示错误的信息
